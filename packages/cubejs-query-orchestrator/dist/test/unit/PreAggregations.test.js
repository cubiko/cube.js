...
describe('PreAggregations', () => {
    ...
    const basicQueryExternal = ramda_1.default.clone(basicQuery);
    basicQueryExternal.preAggregations[0].external = true;
    const basicQueryWithRenew = ramda_1.default.clone(basicQuery);
    basicQueryWithRenew.renewQuery = true;
    const basicQueryExternalWithRenew = ramda_1.default.clone(basicQueryExternal);
    ...
    beforeEach(() => {
        ...
        queryCache = new QueryCache('TEST', mockDriverFactory, 
    ...
    describe('loadAllPreAggregationsIfNeeded', () => {
        ...
        beforeEach(async () => {
            ...
            preAggregations = new PreAggregations('TEST', mockDriverFactory, 
            // eslint-disable-next-line @typescript-eslint/no-empty-function
            () => { }, queryCache, {
                queueOptions: {
                    executionTimeout: 1
    ...
    describe('loadAllPreAggregationsIfNeeded with external rollup and readonly source', () => {
        ...
        beforeEach(async () => {
            const { PreAggregations } = require('../../src/orchestrator/PreAggregations');
            preAggregations = new PreAggregations('TEST', mockDriverReadOnlyFactory, 
            // eslint-disable-next-line @typescript-eslint/no-empty-function
            () => { }, queryCache, {
                queueOptions: {
                    executionTimeout: 1
    ...
    describe('loadAllPreAggregationsIfNeeded with external rollup and externalRefresh true', () => {
        ...
        beforeEach(async () => {
            const { PreAggregations } = require('../../src/orchestrator/PreAggregations');
            preAggregations = new PreAggregations('TEST', () => { throw new Error('The source database factory should never be called when externalRefresh is true, as it will trigger testConnection'); }, 
            // eslint-disable-next-line @typescript-eslint/no-empty-function
            () => { }, queryCache, {
                queueOptions: {
                    executionTimeout: 1
...
//# sourceMappingURL=PreAggregations.test.js.map