...
const QueryOrchestrator_1 = require("../../src/orchestrator/QueryOrchestrator");
class MockDriver {
...
describe('QueryOrchestrator', () => {
    ...
    const queryOrchestrator = new QueryOrchestrator_1.QueryOrchestrator('TEST', (dataSource) => {
        if (dataSource === 'foo') {
    ...
    test('in memory cache', async () => {
        ...
        await queryOrchestrator.fetchQuery(query);
        await queryOrchestrator.fetchQuery(query);
        expect(queryOrchestrator.queryCache.memoryCache.has(queryOrchestrator.queryCache.queryRedisKey(query.cacheKeyQueries.queries[0].slice(0, 2)))).toBe(true);
        expect(queryOrchestrator.queryCache.memoryCache.has(queryOrchestrator.queryCache.queryRedisKey(query.cacheKeyQueries.queries[1].slice(0, 2)))).toBe(false);
        expect(queryOrchestrator.queryCache.memoryCache.has(queryOrchestrator.queryCache.queryRedisKey(query.preAggregations[0].invalidateKeyQueries[0].slice(0, 2)))).toBe(true);
    });
    ...
    test('pre-aggregation version entries', async () => {
        ...
        ], 'stb_pre_aggregations', 'request-id');
...
//# sourceMappingURL=QueryOrchestrator.test.js.map