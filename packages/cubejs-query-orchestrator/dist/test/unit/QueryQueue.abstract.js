...
exports.QueryQueueTest = void 0;
/* globals describe, test, expect, afterAll */
const QueryQueue_1 = require("../../src/orchestrator/QueryQueue");
const QueryQueueTest = (name, options) => {
    describe(`QueryQueue${name}`, () => {
        ...
        let cancelledQuery;
        const queue = new QueryQueue_1.QueryQueue('test_query_queue', {
            queryHandlers: {
        ...
        test('queue driver lock obtain race condition', async () => {
            ...
            await redisClient.addToQueue(keyScore, 'race', time, 'handler', ['select'], priority, { stageQueryKey: 'race' });
            await redisClient.addToQueue(keyScore + 100, 'race2', time + 100, 'handler2', ['select2'], priority, { stageQueryKey: 'race2' });
        ...
        test('activated but lock is not acquired', async () => {
            ...
            const keyScore = time + (10000 - priority) * 1E14;
            await queue.reconcileQueue();
            await redisClient.addToQueue(keyScore, 'activated1', time, 'handler', ['select'], priority, { stageQueryKey: 'race' });
            await redisClient.addToQueue(keyScore + 100, 'activated2', time + 100, 'handler2', ['select2'], priority, { stageQueryKey: 'race2' });
            const processingId1 = await redisClient.getNextProcessingId();
...
exports.QueryQueueTest = QueryQueueTest;
//# sourceMappingURL=QueryQueue.abstract.js.map